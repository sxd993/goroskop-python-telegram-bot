...
async def handle_broadcasts_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(texts.admin_broadcasts_menu(), reply_markup=build_broadcasts_menu_keyboard())
...
async def handle_broadcast_price(message: Message, state: FSMContext):
    ...
    await state.clear()
    campaign = await db.create_campaign(get_settings(message.bot).db_path, title, body, price_kopeks)
...
async def handle_broadcast_launch(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "list":
        ...
        if not campaigns:
            await callback.answer()
            await callback.message.answer(texts.admin_broadcasts_empty(), reply_markup=build_broadcasts_menu_keyboard())
            return
        items = [(_format_campaign_button_text(item), item["id"]) for item in campaigns]
        await callback.answer()
    ...
    if not audience:
        await callback.answer()
        await callback.message.answer("Нет аудитории с оплаченными заказами.", reply_markup=build_broadcasts_menu_keyboard())
        return
    await db.add_campaign_audience(db_path, campaign_id, audience)
    await db.update_campaign_status(db_path, campaign_id, "running")
    await callback.answer()
    await callback.message.answer(texts.admin_broadcast_launch_started(campaign["title"], len(audience)))
    ...
    async def _send():
        ...
        for user_id in audience:
            try:
                text = texts.campaign_offer(campaign["body"], campaign["price_kopeks"] / 100)
                ...
                if msg:
                    sent += 1
                    await db.update_campaign_audience_status(db_path, campaign_id, user_id, "sent", message_id=msg.message_id)
                else:
                    failed += 1
                    await db.update_campaign_audience_status(db_path, campaign_id, user_id, "failed", error="Delivery failed")
            except Exception as exc:
                logger.exception("Campaign send failed user_id=%s campaign_id=%s", user_id, campaign_id)
                failed += 1
                await db.update_campaign_audience_status(db_path, campaign_id, user_id, "failed", error=str(exc))
        ...
        await send_message_safe(
            ...
            texts.admin_broadcast_launch_finished(sent, failed, interested, declined),
            reply_markup=build_broadcasts_menu_keyboard(),
        )
...
async def handle_broadcast_responses(callback: CallbackQuery, state: FSMContext):
    ...
    if not responses:
        await callback.answer()
        await callback.message.answer(texts.admin_broadcast_responses_empty(), reply_markup=build_broadcasts_menu_keyboard())
        return
    ...
    for resp in responses:
        ...
        lines.append("—" * 10)
    await callback.answer()
    await callback.message.answer("\n\n".join(lines), reply_markup=build_broadcasts_menu_keyboard())
