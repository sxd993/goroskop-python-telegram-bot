...
from app.features.admin.keyboards import (
    ...
    ADMIN_STATS_KIND_PREFIX,
    ADMIN_STATS_MONTH_OPEN_PREFIX,
    ADMIN_STATS_MONTHS_PAGE_PREFIX,
    ADMIN_STATS_YEAR_OPEN_PREFIX,
    ADMIN_STATS_YEARS_PAGE_PREFIX,
    build_admin_menu,
    build_admin_stats_kind_keyboard,
    build_admin_stats_month_detail_keyboard,
    build_admin_stats_months_keyboard,
    build_admin_stats_year_detail_keyboard,
    build_admin_stats_years_keyboard,
)
...
async def _show_years_page(callback: CallbackQuery, *, page: int) -> None:
    if page < 1:
        await callback.answer(texts.invalid_choice(), show_alert=True)
        return

    settings = get_settings(callback.bot)
    offset = (page - 1) * YEARS_PAGE_SIZE
    raw = await db.fetch_paid_years_page(settings.db_path, limit=YEARS_PAGE_SIZE + 1, offset=offset)
    if not raw:
        await callback.answer(texts.admin_stats_empty() if page == 1 else texts.invalid_choice(), show_alert=page != 1)
        if page == 1:
            await edit_or_send(callback, texts.admin_stats_empty(), reply_markup=build_admin_menu())
    ...
    has_next = len(raw) > YEARS_PAGE_SIZE
    years = raw[:YEARS_PAGE_SIZE]

    items: list[tuple[str, str]] = [(f"📅 {year}", year) for year in years]
    markup = build_admin_stats_years_keyboard(items, page=page, has_prev=page > 1, has_next=has_next)
    await callback.answer()
    await edit_or_send(callback, texts.admin_stats_choose_year(page), reply_markup=markup)
...
async def handle_admin_stats(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.answer()
    await edit_or_send(callback, texts.admin_stats_title(), reply_markup=build_admin_stats_kind_keyboard())


@router.callback_query(F.data.startswith(f"{ADMIN_STATS_KIND_PREFIX}:"))
async def handle_admin_stats_kind(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.bot, callback.from_user.id):
        await callback.answer(texts.admin_forbidden(), show_alert=True)
        return
    await state.clear()
    ...
    if kind == "year":
        await _show_years_page(callback, page=1)
        return
    if kind == "month":
...
@router.callback_query(F.data.startswith(f"{ADMIN_STATS_YEARS_PAGE_PREFIX}:"))
async def handle_admin_stats_years_page(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.bot, callback.from_user.id):
        await callback.answer(texts.admin_forbidden(), show_alert=True)
        return
    await state.clear()
    ...
    try:
        ...
        await callback.answer(texts.invalid_choice(), show_alert=True)
        return
    await _show_years_page(callback, page=page)
...
@router.callback_query(F.data.startswith(f"{ADMIN_STATS_YEAR_OPEN_PREFIX}:"))
async def handle_admin_stats_year_open(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.bot, callback.from_user.id):
        await callback.answer(texts.admin_forbidden(), show_alert=True)
        return
    await state.clear()
    ...
    settings = get_settings(callback.bot)
    rows = await db.fetch_year_sales_breakdown(settings.db_path, year=year)
    if not rows:
        await callback.answer()
        await edit_or_send(
            callback,
            texts.admin_stats_year_empty(year),
            reply_markup=build_admin_stats_year_detail_keyboard(page=page),
        )
        return

    total_count = 0
    total_amount = 0
    lines = [texts.admin_stats_year_title(year)]
    for sign, count, total in rows:
        sign_name = SIGNS_RU.get(sign, sign)
        emoji = SIGN_EMOJI.get(sign, "🔮")
        lines.append(f"{emoji} {sign_name}: {count} шт. / {total/100:.0f} ₽")
