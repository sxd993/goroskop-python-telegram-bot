...
1. Пользователь отправляет `/start`. Бот сканирует `MEDIA_DIR` и предлагает выбрать тип расклада: годовой (`media/year/YYYY/sign.ext`) или месячный (`media/month/YYYY/MM/sign.ext`).
2. Для годового расклада: выбор года → выбор знака → создание заказа и оплата.
3. Для месячного расклада: выбор года → выбор месяца → выбор знака → создание заказа и оплата.
